...
(function() {
  ...
  // 首先获取当前 viewport 的宽和高，先计算出单个项的宽度(设置的宽度+空隙宽度)
  // 然后算出一行总共可以放几列，同时设置每列初始高度为 0，viewportHeight 先存起来，后面使用
  ...
  /**
   * 获取内容项，先加入到 fragment，然后检查每个图片的高度
   */
  ...
  /**
   * 设置新的 pins 的位置： 对 pins 进行迭代
   * 首先找出已渲染项中高度最小的索引，设置当前迭代项的 left 和 top
   * 更新高度最小那列的最新高度值，用于给下一个项进行计算使用
   * 然后调用 setWaterFall
   */
