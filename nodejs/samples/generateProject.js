...
const getDirNameList = pathName => {
  let dirList = []
  let allList = fs.readdirSync(pathName) // 同步获取指定目录的文件/文件夹信息
  for (let item of allList) {
    if (fs.statSync(item).isDirectory()) { // 分析指定文件/文件夹信息，判定是否是文件夹
      dirList.push(item)
    }
  }
  return dirList
...
const generatePackageJsonFile = projectName => {
  ...
  fs.writeFileSync(`./${projectName}/package.json`, JSON.stringify(jsonContent))
}
